...
variable "access_identities_views" {
...
variable "consumo" {
  description = "Variable to identify if a dataset is of type consumption."
  type        = string
  default     = ""
  validation {
    condition     = var.consumo == "" || var.consumo == "n1" || var.consumo == "n2" || var.consumo == "n3"
    error_message = "The consumo value must be a valid consumption type: n1, n2, n3 or empty."
  }
}
...
variable "views" {
  description = "View definitions. \n <a name=friendly_name:></a>[friendly_name:](#friendly_name:) A descriptive name for the table. \n <a name=labels:></a>[labels:](#labels:) A mapping of labels to assign to the resource. \n <a name=query:></a>[query:](#query:) A query that BigQuery executes when the view is referenced. \n <a name=use_legacy_sql:></a>[use_legacy_sql:](#use_legacy_sql:) Specifies whether to use BigQuery's legacy SQL for this view. The default value is true. If set to false, the view will use BigQuery's standard SQL. \n <a name=deletion_protection:></a>[deletion_protection:](#deletion_protection:) Whether or not to allow Terraform to destroy the instance. Unless this field is set to false in Terraform state, a terraform destroy or terraform apply that would delete the instance will fail."
  type = map(object({
    friendly_name       = string
    labels              = optional(map(string))
    query               = string
