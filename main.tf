...
locals {
  prefix = join("_", tolist(["go", lower(var.tags.region), var.tags.enterprise, var.tags.account, var.tags.system, ""]))
  ...
  identities_view = {
    for k, v in local.access_view : k => try(
      zipmap(
        ...
        split("|", var.access_identities_views[k])
      ),
      { project = null, dataset = null, table = null }
    )
  }

  dataset_id = var.location == "europe-west4" ? (var.consumo != "" ? format(
    "%s_%s_%s_ds%.2d",
    var.id,
    "euw4",
    var.consumo,
